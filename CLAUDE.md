...
本仓库共三个文件：`index.html`（前端全部）、`AppsScript.gs`（后端全部）、本文档。
...
真正多余的只有每行 1 次的 `getValue`（ID 是否为空的检查），共 5 次读——因为上面几行已经把空 ID 覆盖掉了，只能重读单元格。可以用 `var needId = !r.id;` 提前记录来消除，尚未做。
...
旧文档记载「项目有 9 套测试，约 290 项断言」，文件名为 `gastest.mjs` / `savtest.mjs` / `bondtest.mjs` / `e2etest.mjs` / `test2/4/5/6.mjs` / `uitest.mjs`。**这些文件不在本仓库中，本机也搜不到。** 在它们恢复之前，「改完代码跑一遍全部测试」这条要求无法照字面执行。

其中 `e2etest.mjs` 的设计意图值得保留——旧文档记的教训：后端测后端、前端测前端各自通过，但中间的字段契约从没对接过，曾漏掉后端返回 `bal`、前端读 `balance`，导致储蓄全显示 0。

### 目前可用的验证手段

在没有正式测试工程的情况下，以下方法验证过本轮改动，可以复用：
...
⚠️ **已知遗留问题**：`monthly` / `expense` / `monthEnd` 至今仍用 `new Date(字符串)`。如果 Ledger 的日期列存的是**文本**而非真正的日期格式，这些月度汇总在月份边界上会偏一天。若日期列是真日期格式则两条路径一致，不受影响。尚未修复。

### 3. `ASOF` 显示的是今天，不是数据时点

`const ASOF = new Date().toISOString().slice(0,10)` —— 永远是今天，且用的是 **UTC**。亚利桑那是 UTC-7，所以每天下午 5 点后右上角就会显示成明天。

引入缓存后这里会出现矛盾：右上角写「8/29」，底部徽标写「数据为 8/27」。尚未修复。

### 4. `DATA` 里还有硬编码的种子数据

首次安装（无缓存）时，国债会先闪一下 `T-Notes 10000` / `I Bond 8000`，现金闪 `Chase 10000`——都标着 `updated:"2026-07-31"`。退休里的 `Roth IRA 7515` 和 HSA 的两条更是**永远不会被同步覆盖**。

`DATA.cd`（储蓄）反而是对的：不留种子，宁可显示 $0。
...
9. 第八节列的三个未修复项：日期解析不一致、`ASOF` 显示、硬编码种子
