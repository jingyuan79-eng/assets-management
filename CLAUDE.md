...
本仓库：`index.html`（前端全部）、`AppsScript.gs`（后端全部）、本文档，以及 9 套测试（见第七节）。
...
9 套测试，**291 项断言**，用 jsdom + 假的 Sheets API 跑。**改代码后必须全部跑一遍。**
...
9 套全部通过 `readFileSync` 直接读仓库里的真实源文件，所以改动会立即被检验。测试只断言**公开行为**，不引用内部函数名——重构内部实现不会误伤。

> 教训：后端测后端、前端测前端各自通过，但**中间的接口从没对接过**，曾漏掉一次字段名不一致（后端返回 `bal`，前端读 `balance`），导致储蓄全显示 0。`e2etest.mjs` 就是为此加的。

### ⚠️ 已知覆盖盲区

以下几处是 2026-08 的改动，**这 9 套测试都没有针对性断言**，改动它们时要格外小心：

- **70 天流水窗口**（第二节第 9 条）—— 没有断言检查 `ledger` 返回的日期范围
- **`sheetCache_v1` 首屏缓存**（第二节第 10 条）—— 所有前端测试都在 `beforeParse` 里 `localStorage.clear()`，等于绕开了缓存注水路径；徽标的 stale 态也没测
- **`writeSavRow` / `writeBondRow` 整行回写**（第八节第 1 条）—— 没有断言检查「不该动的列有没有被动」

好消息：`test6.mjs:79` 有一条精确的 `余额=锚点+流水` 断言（期望 10322.51，容差 0.01）。本轮那个 `cash.balance` 差 $89 的回归，如果当时这套测试在手，**大概率会被它抓到**。
...
3. **测试覆盖盲区** — 70 天窗口、首屏缓存、整行回写三处无断言，见第七节
