/**
 * 资产驾驶舱 · Google Apps Script 后端  V6.3  (2026-09-02)
 *
 ...
 *
 * V6.3 2026-09-02  房产不再留写死的种子
 *   · 前端 PROPERTY 的初始值全部清零（同 cd / bond / cash）。房产已由
 *     Sheet 驱动，本地再留一份写死的数字，只会在断网或 Sheet 尚未填写时
 *     显示一个「看起来完全合理」的假余额 —— 这种假数最难被发现
 *   · amortizeNow 在锚点为空时直接返回，不再产生非数字
 *   · 还款进度条在原贷金额为 0 时不再除零
 *   （纯前端改动，后端未变，版本号跟随前端保持一致）
 *
